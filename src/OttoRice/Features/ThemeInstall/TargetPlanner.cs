...
/// </summary>
public sealed class TargetPlanner(WindowsTerminalLocator wtLocator, Func<string, string>? pathExpander = null)
{
    ...
    public Result<List<FileOperation>> Build(RiceManifest manifest, string themeDirectory)
    {
            ...
            switch (target.Action)
            {
                ...
                case "override" when File.Exists(sourcePath):
                    ...
                    var match = app.ConfigPaths.FirstOrDefault(p =>
                        ...
                        ?? app.ConfigPaths.FirstOrDefault();
        ...
        static Result<List<FileOperation>> Fail(string error) => Result<List<FileOperation>>.Fail(error);
    }
}
